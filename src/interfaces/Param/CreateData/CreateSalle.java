...
public class CreateSalle {
	...
	public CreateSalle(ParamPannel param) {
		this.param=param;
		initWithDB();
		initialize();
		initializeSalle();
	...
	public void addAll() {
		Label numLabel=new Label("Numero de salle :");
		numLabel.setStyle("-fx-font-weight: bold;");
		TextField numEnter = new TextField(String.valueOf(this.numSalle));
		HBox numBox = new HBox(numLabel,numEnter);
		numBox.setSpacing(20);
		...
		see.setOnAction(e->{
			...
			this.visu.setAll(numEnter.getText(),Integer.parseInt(capEnter.getText()),this.nature,this.currentGrp);
			this.addField.clear();
		...
		secBox.getChildren().addAll(numBox,capBox,nature(),groupe(),seeBox,buttonBox);
	}
